...
1. In 1-2 sentences, explain what React's `useRouteMatch` hook is used for.
    ...
    A: useRouteMatch is used for rendering routes that are an exact match when some action has occurred. It filters out other options and makes sure to take you to the right place without use of a switch or writing out a new route.

2. How would you explain form validation to someone who has never programmed before?

    A: Form validation is a process that looks at the the given inputs that a user is putting in and making sure it meets all the conditions needed before moving forward (e.g. all the different inputs are filled out, min character length, types of characters accepted, etc.)

3. In 1-2 sentences, define end to end testing.

    A: End-to-end testing is a tpe of testing that looks at the entire user experience. It tests to make sure that all the pages are navigating to where they need to go, that pages are rendering correctly, and that forms and links are working as expected without bugs or generating unexpected errors that the user would bump into.
